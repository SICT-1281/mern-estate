      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (
            ...
            <span className='text-teal-600 font-mono'>{`Uploading ${filePerc}%`}</span>
          ) : filePerc === 100 ? (
            <span className='text-teal-400 font-mono'>Image successfully uploaded!</span>
          ) : (
            ''
          )}
        </p>
        <input type='text' placeholder='username' defaultValue={currentUser.username} id='username'
        className='focus:outline-noneborder p-3 rounded-lg bg-neutral-600 font-mono' onChange={handleChange}/>
        <input type='email' placeholder='email' defaultValue={currentUser.email} id='email'
        className='focus:outline-noneborder p-3 rounded-lg bg-neutral-600 font-mono' onChange={handleChange}/>
        <input type='password' placeholder='password' id='password'
        className='focus:outline-noneborder p-3 rounded-lg bg-neutral-600 font-mono' onChange={handleChange}/>
        <button disabled={loading}
        className='bg-teal-400 rounded-lg font-mono p-3 uppercase hover:opacity-95 disabled: opacity-80'>
